(function () {
  ...
  Lampa.Listener.follow('full', function(e){
    ...
    .text()
		...
		render.find('.cardify-meta').removeClass('cardify-show');
render.find('.cardify-overview').removeClass('cardify-show');
render.find('.cardify-info').removeClass('cardify-show');



        render.find('.full-start-new__title')
        .html(
            '<img style="max-height:220px;max-width:800px;" src="' +
            Lampa.TMDB.image('/t/p/w500' + logo.replace('.svg','.png')) +
            '"/>'
        );
    setTimeout(function(){
    render.find('.full-start-new__title img')
    .addClass('logo-show');
},400);

setTimeout(function(){
    render.find('.cardify-meta').addClass('cardify-show');
},700);

setTimeout(function(){
    render.find('.cardify-overview').addClass('overview-show');
},1000);

setTimeout(function(){
    render.find('.cardify-info').addClass('info-show');
},1300);

        render.find('.cardify-overview')
        .text(data.overview || '');
		var year = '';
...
if(data.production_companies &&
   ...
   data.production_companies[0].logo_path){
    ...
    studioLogo =
    '<img src="https://image.tmdb.org/t/p/h100' +
    data.production_companies[0].logo_path +
    '"style="height:1em;width:auto;vertical-align:middle;margin-right:2px;">';
}	
...
setTimeout(function(){
    var age = render.find('.full-start__pg').text().trim();

var ageBadge = age
    ? '<span style="display:inline-block;padding:2px 8px;border:1px solid rgba(255,255,255,.4);border-radius:6px;font-size:.85em;margin-left:8px;">'+age+'</span>'
    : '';

render.find('.cardify-meta').html(
    studioLogo +
    ' • Фильм • ' +
    genres +
    ageBadge
